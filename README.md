...
> [!WARNING]
> 本项目会向目标进程加载本地 DLL，并在目标进程中 Hook `RegisterHotKey`。由于发布版本未签名，Microsoft Defender SmartScreen 可能将其标记为未识别应用并发出警告；其他安全软件也可能检测或阻止这类行为。
...
* 自动识别目标进程 x86 / x64 架构并选择对应运行组件；
* 实时监控目标进程，并显示“等待程序启动”、“正在启用拦截”和“拦截已生效”等状态；
* 修改快捷键策略后需重启目标应用，已注册的快捷键不会被主动撤销；
* 支持系统托盘运行和当前用户登录时启动，不安装 Windows 服务；
* 使用原生 Windows 界面，显示应用图标、路径、策略和运行状态；
...
启用或停用规则不会关闭已运行的目标应用；重新启动应用后更改才能完全生效。

快捷键策略变更后，正在运行的目标应用需要重启；程序不能撤销目标应用已经注册的快捷键。
...
更多规则管理和故障排查说明见 [用户指南](docs/USER_GUIDE.md)。

## 开发与构建

从源码构建和运行测试的说明见 [构建、测试和打包](docs/BUILD.md)。
...
先查看 `%LOCALAPPDATA%\HotkeyBlocker\logs\hkb.log` 中的错误码，并确认目标应用权限和安装文件完整。不要通过关闭系统保护或降低权限边界来解决问题。

### x64 安装包为什么还需要 x86 组件？

为处理 32 位目标应用，x64 安装包同时包含 x86 组件。
...
欢迎提交 Issue 和 Pull Request。问题反馈、功能建议和代码贡献方式见 [贡献指南](CONTRIBUTING.md)。

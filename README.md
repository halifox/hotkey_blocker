...
有些应用会注册全局快捷键，却没有提供关闭或修改入口。Hotkey Blocker 只针对你选定的进程，在进程内部拒绝标准 `RegisterHotKey` 调用；其他应用和普通键盘输入不受此规则直接影响。
...
规则列表会显示目标路径、启用状态和拦截状态，包括等待启动、正在启用、拦截已生效、部分拦截生效、需要重启、启用失败、路径不存在和进程监控不可用等状态。
...
主窗口使用 Win32/WTL 和系统文件选择器，规则列表显示目标程序图标、路径、启用状态及运行状态，便于直接定位问题。
...
2. 确认规则处于启用状态。
3. 如果目标应用已经运行，请完全退出并重新启动它。
4. 当状态显示“拦截已生效”后，该进程后续的标准全局快捷键注册会被拒绝。
...
```text
      ...
      HotkeyHook64.dll       HotkeyBlockerInjector32.exe
             ...
             Hook user32!RegisterHotKey
                           │
                           ▼
         标准全局快捷键注册返回失败（ERROR_ACCESS_DENIED）
```
...
- Hook DLL 使用 Detours 拦截目标进程内部的 `user32!RegisterHotKey`，让标准全局快捷键注册返回失败。
...
| 路径 | 用途 |
| --- | --- |
| `%LOCALAPPDATA%\HotkeyBlocker\config.ini` | 规则路径、规则类型和启用状态 |
...
目标进程在规则启用前已经运行。程序不会强制卸载目标进程中的 DLL，因此请完全退出并重新启动该应用。
...
先查看 `%LOCALAPPDATA%\HotkeyBlocker\logs\hkb.log`，检查目标进程架构、权限、Hook DLL 和 x86 辅助程序是否完整。不要通过关闭系统保护或降低权限边界来解决问题。
